...
def test_onnx_prediction():
    ...
    with TestClient(app) as client:
        ...
        print("Sending POST request to ONNX local /api/predict...")
        ...
        if response.status_code == 200:
            ...
            print("All ONNX response attributes successfully verified!")
            
            # Test PDF generation endpoint
            print("Sending POST request to ONNX local /api/export-pdf...")
            pdf_payload = {
                "disease": res["disease"],
            ...
            pdf_response = client.post("/api/export-pdf", json=pdf_payload)
            print(f"PDF Status Code: {pdf_response.status_code}")
            assert pdf_response.status_code == 200
            assert len(pdf_response.content) > 1000  # Should be a non-empty PDF file
            print("PDF generation successfully verified end-to-end!")
